...
export class Effects {
  ...
  // An energy beam. Defaults to a slim bolt (the mage's thunder). The blaster
  // passes `mega: true` for a white-hot core inside a wide coloured glow, light
  // along its whole length, and a hefty screen shake.
  beam(start, end, color = 0x54d2ff, options = {}) {
    ...
    const coreRadius = mega ? 0.18 : 0.07;
    const glowRadius = mega ? 0.55 : 0;
    const ttl = mega ? 0.22 : 0.16;
    ...
    group.add(new THREE.Mesh(new THREE.CylinderGeometry(coreRadius, coreRadius, length, mega ? 10 : 8, 1, true), coreMat));
